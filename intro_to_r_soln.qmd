...
```{r}
# your code here:
mod_diamonds <- lm(price ~ exp(carat) * cut * color * clarity, data = diamonds)
mod_diamonds |> summary()
```
...
# your code here:
diamonds |> 
  ggplot(aes(x = exp(carat), y = price, col = cut, shape = color)) +
  geom_point(alpha = .1) +
  facet_wrap(~ clarity) +
  coord_cartesian(xlim = c(0, exp(3))) +
  theme_bw() +
